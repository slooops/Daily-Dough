  ...
  try {
    ...
    try {
      ...
      while (hasMore) {
        ...
        // Check if we're getting initial update pending (PRODUCT_NOT_READY scenario)
        if (
          !cursor &&
          added.length === 0 &&
          modified.length === 0 &&
          ...
          console.log(
            "⏳ Transactions not ready yet - returning initializing status"
          );

          // Update sync state to show we're waiting for initial data
          await saveSyncState({
            userId,
            itemId: item.id,
            cursor: null,
